...
class TestMaskFactory(TestCase):
    def test_mask_factory_reads_file(self):
        ...
        mask = MaskFactory.create_mask('resources/mask01.txt')
    ...
    def test_mask_maze_raises_error_when_different_length(self):
        ...
        MaskFactory.mask_maze(maze, mask)


    def test_mask_maze_masks_maze_correctly(self):
        maze = Maze(20, 5, 5)
        mask = MaskFactory.create_mask('resources/mask02.txt')

        maze = MaskFactory.mask_maze(maze, mask)

        for row_idx in range(len(mask)):
            row = mask[row_idx]
            for col_idx in range(len(row)):
                self.assertEquals(row[col_idx], maze.cells[col_idx][row_idx].masked, 'col_idx: {}, row_idx: {}'.format(col_idx, row_idx))
